  ...
  "records": [
    {
      ...
      "dimensions": {
        "intent_fidelity": "passed | deviated | blocked | pending_human",
        "philosophy_consistency": "passed | deviated | blocked | pending_human",
        "baseline_compliance": "passed | deviated | blocked | pending_human",
        "acceptance_achievement": "passed | deviated | blocked | pending_human"
      },
...
- `verdict`：总判定，四选一
- `timestamp`：ISO 8601 时间戳
- `summary`：验证摘要——具体证据（"对照意图叙事第 X 段，实现中缺失了 Y"），不是"看起来没问题"
- `dimensions`：四个维度的判定结果
